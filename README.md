...
- **运营数据**：弹性网格卡片布局，数值/柱状图/折线图/饼图，日期筛选栏，支持拖拽排序
...
│   │   └── Middleware/            # 异常/审计中间件
│   └── HospitalStats.Api.Tests/   # xUnit 测试（180 个）
├── deploy/                        # 部署脚本和发布输出
├── docs/                          # 本地文档（不上传）
├── LICENSE                        # MIT 许可证
├── CLA.md                         # 贡献者许可协议
...
`25e864c`（2026-06-18，完整哈希 `25e864c19c7d55ce0c44188c5f5c994c941a17bf`）是区分社区版与企业版之前的最后一版，已包含在 `master` 历史中。这一版可以独立编译运行：查询执行仍在 `QueryExecutionService`，没有 `Extensions/` 插件接口，也没有独立的 `HospitalStats.QueryEngine`。

要在这一版上开发，从该提交新建分支，不要改动当前 `master`：

```bash
git checkout -b <分支名> 25e864c
```

这条分支是分版前的单体程序，新功能直接写在本仓库。现有企业版仓库按当前社区版接口对接，接不上 `25e864c`。

该提交之后的改动不会进入这条分支：

| 提交 | 日期 | 内容 |
|------|------|------|
| `dfe8beb` | 2026-06-29 | 社区版 / 企业版拆分：扩展接口与前端企业版注入 |
| `ddbb029` | 2026-07-01 | 抽出 `HospitalStats.QueryEngine` |
| `b047928` | 2026-07-09 | 企业版子菜单 |

以后若要把这条分支并回 `master`，冲突会集中在 `Program.cs`、查询执行和前端菜单加载。
